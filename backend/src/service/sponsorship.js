...
exports.save = async ({payload}) => {
    ...
    if (!sponsorshipData.sponsorData?.firstName || !sponsorshipData.sponsorData.firstName.trim()) {
        throw new CustomError("Sponsor first name is required", 400);
    }

    if (!sponsorshipData.sponsorData?.lastName || !sponsorshipData.sponsorData.lastName.trim()) {
        throw new CustomError("Sponsor last name is required", 400);
    }

    if (!sponsorshipData.sponsorData?.email || !sponsorshipData.sponsorData.email.trim()) {
...
exports.updatePaymentStatus = async ({sponsorshipId, paymentStatus, stripePaymentIntentId = null}) => {
    if (!sponsorshipId) {
        throw new CustomError("Sponsorship ID is required", 400);
    }
