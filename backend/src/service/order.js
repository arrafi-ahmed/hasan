...
exports.updatePaymentStatus = async ({orderId, paymentStatus, stripePaymentIntentId = null}) => {
    if (!orderId) {
        throw new CustomError("Order ID is required", 400);
    }
    ...
    `;
    const result = await query(sql, [paymentStatus, stripePaymentIntentId, orderId]);
...
exports.createOrderWithPayment = async ({registrationId, items, totalAmount}) => {
    // Validate required fields
    if (!registrationId) {
        throw new CustomError('Registration ID is required', 400);
    }
    if (!items || !Array.isArray(items) || items.length === 0) {
        throw new CustomError('Items are required and must be a non-empty array', 400);
    }
    ...
    for (const item of items) {
        const ticket = await ticketService.getTicketById({ticketId: item.ticketId});
    ...
    if (!finalTotalAmount || isNaN(finalTotalAmount)) {
        ...
        items.forEach((item, index) => {
            ...
            if (isNaN(unitPrice) || unitPrice < 0) {
                throw new CustomError(`Invalid unitPrice for item ${index}: ${item.unitPrice || item.price}`, 400);
            }

            if (isNaN(quantity) || quantity <= 0) {
                throw new CustomError(`Invalid quantity for item ${index}: ${item.quantity}`, 400);
            }
...
exports.updatePaymentStatusWithStockUpdate = async ({orderId, paymentStatus, stripePaymentIntentId}) => {
    if (!orderId) {
        throw new CustomError('Order ID is required', 400);
    }
    if (!paymentStatus) {
