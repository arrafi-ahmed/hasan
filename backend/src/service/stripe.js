...
exports.createPaymentIntent = async ({
    ...
    const tickets = await ticketService.getTicketsByEventId({ eventId: savedRegistration.eventId });
...
exports.getRegistrationFromPaymentIntentMetadata = async (paymentIntentId) => {
    ...
    if (paymentIntent.metadata.sessionId) {
        try {
            ...
            if (!sessionData) {
                console.error('No temporary registration data found for session:', paymentIntent.metadata.sessionId);
                throw new CustomError('No registration data found in payment intent', 400);
            }


            if (!sessionData.attendees || !Array.isArray(sessionData.attendees) || sessionData.attendees.length === 0) {
            ...
            const totalAmount = sessionData.selectedTickets.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
            ...
            console.error('Error processing payment intent with temporary storage:', error);
            ...
            throw new CustomError(`Failed to process registration from payment intent: ${error.message}`, 500);
        }
    }
...
exports.createSponsorshipPaymentIntent = async ({ packageId, amount, currency, sponsorEmail, eventId }) => {
    if (amount <= 0) {
        return { clientSecret: "no-stripe" };
    }
...
exports.createSecurePaymentIntent = async ({ attendees, selectedTickets, registration }) => {
    ...
    for (const attendee of attendees) {
        if (!attendee.email || !attendee.firstName || !attendee.lastName || !attendee.phone) {
            throw new CustomError('All attendees must have email, first name, last name, and phone', 400);
        }
    }

    // Check if any attendee already exists for this event
    for (const attendee of attendees) {
        const existingRegistration = await registrationService.getRegistrationByEmail({
            email: attendee.email,
    ...
    try {
        ...
        for (const frontendItem of selectedTickets) {
            ...
            if (frontendItem.quantity <= 0 || frontendItem.quantity > 10) {
                throw new CustomError(`Invalid quantity for ticket ${dbTicket.title}`, 400);
            }

            // Validate stock availability
            if (dbTicket.current_stock < frontendItem.quantity) {
            ...
            validatedItems.push({
                ...
                currentStock: dbTicket.current_stock
            });
        }
...
exports.checkPaymentStatus = async ({ paymentIntent }) => {
    ...
    try {
        ...
        if (stripePaymentIntent.metadata.processed === 'true' && stripePaymentIntent.metadata.registrationId) {
...
exports.webhook = async (req) => {
    ...
    switch (eventType) {
        ...
        case "payment_intent.succeeded":
            ...
            console.log('Payment Intent ID:', paymentIntentSucceeded.id);

            // Validate metadata types - ensure all values are strings
            const metadata = paymentIntentSucceeded.metadata;
            for (const [key, value] of Object.entries(metadata)) {
                if (typeof value !== 'string') {
                    console.error(`Invalid metadata type for key ${key}:`, value, typeof value);
            ...
            if (paymentIntentSucceeded.metadata.sessionId) {
                ...
                try {
                    ...
                    if (!sessionData) {
                        console.error('No temporary registration data found for session:', paymentIntentSucceeded.metadata.sessionId);
                        throw new CustomError('No registration data found in payment intent', 400);
                    }

                    // Destructure all table data from temp registration
                    const { attendees, registration, selectedTickets, orders, eventId } = sessionData;
                    ...
                    if (orders) {
                        ...
                        const orderData = {
                            orderNumber: orderService.generateOrderNumber(),
                            totalAmount: selectedTickets.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0),
                            currency: defaultCurrency.code,
                            paymentStatus: 'paid',
