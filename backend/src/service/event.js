...
exports.save = async ({payload, files, currentUser}) => {
    ...
    if (payload.landingConfig) {
        newEvent.landingConfig = typeof payload.landingConfig === 'string'
    ...
    if (shouldCreate || (payload.slug && payload.slug !== newEvent.slug)) {
        if (payload.slug && payload.slug.trim()) {
            ...
            if (!isUnique) {
                throw new CustomError("Slug already exists. Please choose a different one.", 400);
            }
    ...
    if (id) {
        const sql = `
            ...
            UPDATE SET
                ...
                RETURNING *
        `;
        const values = [id, extrasData.name, extrasData.description, extrasData.price, extrasData.currency, JSON.stringify(extrasData.content), extrasData.eventId];
        const result = await query(sql, values);
        return result.rows[0];
    } else {
        const sql = `
        ...
        `;
        const values = [extrasData.name, extrasData.description, extrasData.price, extrasData.currency, JSON.stringify(extrasData.content), extrasData.eventId];
        const result = await query(sql, values);
        return result.rows[0];
    }
};
