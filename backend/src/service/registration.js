...
exports.bulkImportAttendee = async ({files, eventId, clubId}) => {
    ...
    if (!worksheet) {
        throw new CustomError('Excel file must contain at least one worksheet', 400);
    }
    ...
    for (const row of rows) {
        ...
        if (!fullName || fullName.trim() === '') {
            throw new CustomError(`Name is required for attendee with email: ${email}`, 400);
        }
    ...
    for (const attendee of attendees) {
        if (emailSet.has(attendee.email)) {
            throw new CustomError(`Duplicate email found in import: ${attendee.email}`, 400);
        }
    ...
    if (attendees.length) {
        ...
        const registrationValues = [eventId, clubId, JSON.stringify(attendees[0].additionalFields), true];
    ...
    savedRegistrations.forEach(async (registration) => {
        try {
            ...
            emailService.sendTicket({registrationId: registration.id})
                .catch(error => {
                    console.error(`Failed to send email for registration ${registration.id}:`, error);
                });
        } catch (error) {
            console.error(`Failed to queue email for registration ${registration.id}:`, error);
        }
    });
...
exports.defaultSave = async ({payload}) => {
    ...
    const tickets = await ticketService.getTicketsByEventId({eventId: payload.eventId});
    ...
    if (payload.registrationData) {
        ...
        const attendee = {
            ...
            lastName: payload.registrationData.name?.split(' ').slice(1).join(' ') || '',
...
exports.save = async ({eventId, additionalFields, status = false}) => {
    ...
    const result = await query(sql, [eventId, JSON.stringify(additionalFields || {}), status]);
...
exports.searchAttendees = async ({eventId, searchKeyword, sortBy = "registration"}) => {
...
exports.completeFreeRegistration = async ({payload}) => {
    ...
    if (hasPaidTickets) {
        throw new CustomError('Cannot process free registration with paid tickets', 400);
    }
    ...
    try {
        ...
        const totalAmount = selectedTickets.reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
...
exports.getFreeRegistrationConfirmation = async ({registrationId}) => {
    ...
    try {
        ...
        const registration = await exports.getRegistration({registrationId, isLoggedIn: false});
        if (!registration) {
            throw new CustomError('Registration not found', 404);
        }

        // 2. Get order data for this registration
        const order = await orderService.getOrderByRegistrationId({registrationId});
        if (!order) {
        ...
        const attendeesWithTickets = await Promise.all(
            registration.attendees.map(async (attendee) => {
                if (attendee.ticketId) {
                    const ticket = await ticketService.getTicketById({ticketId: attendee.ticketId});
                    return {
                        ...attendee,
        ...
        throw new CustomError('Failed to retrieve free registration confirmation data', 500);
    }
};
