...
<template>
  <v-container class="scanner-container" fluid>
    <!-- Header Section -->
    <v-row class="mb-4">
      <v-col cols="12">
        <PageTitle :subtitle="event?.name" title="Scanner">
          <template #actions>
            <v-btn
    ...
    <!-- Scanner Section -->
    <v-row v-if="showScanner" class="mb-6">
          ...
          <v-card-text class="pa-0">
            <div class="scanner-viewport">
              ...
              <div v-if="cameraLoading" class="camera-loading">
                ...
                <p class="text-body-1 mt-3 text-medium-emphasis">Initializing camera...</p>
              </div>

              <!-- Camera Error State -->
              <div v-else-if="cameraError" class="camera-error">
                <v-icon color="error" icon="mdi-camera-off" size="64" />
                <p class="text-body-1 mt-3 text-error">Camera failed to initialize</p>
                <v-btn color="primary" size="small" variant="outlined" @click="retryCamera">
                  Retry Camera
                </v-btn>
              </div>
              ...
              <div v-if="!cameraLoading && !cameraError">
                <div class="scanner-overlay">
                  ...
                  <div class="scanner-instructions">
                    <v-icon class="mr-1" color="white" icon="mdi-qrcode-scan" size="24" />
              ...
              <div v-if="scanSuccess" class="scan-feedback success">
                <v-icon color="success" icon="mdi-check-circle" size="64" />
                <p class="text-success text-h6 mt-2">Success!</p>
              </div>

              <div v-if="scanError" class="scan-feedback error">
                <v-icon color="error" icon="mdi-alert-circle" size="64" />
                <p class="text-error text-h6 mt-2">Error!</p>
              </div>
            </div>
          </v-card-text>
        </v-card>
    ...
    <v-row v-if="Object.keys(result).length > 0">
      <v-col class="mx-auto" cols="12" lg="8" md="10">
        <!-- Attendee Details -->
        <v-card v-if="scannerVariant === 'main' && result.registrationData" class="mb-4">
          <v-card-title class="d-flex align-center pa-4">
            <v-icon class="mr-2" color="primary" icon="mdi-account-details" size="24" />
          ...
          <v-card-text class="pa-4">
            ...
            <v-expand-transition>
              ...
              >
                ...
                <h4 class="text-subtitle-1 font-weight-medium mb-3">Additional Information</h4>
                <v-row>
                  <v-col
        ...
        >
          <v-card-title class="d-flex align-center pa-4">
            <v-icon class="mr-2" color="secondary" icon="mdi-ticket-confirmation" size="24" />
          ...
          <v-card-text class="pa-4">
            ...
            <v-divider class="my-4" />
            <h4 class="text-subtitle-1 font-weight-medium mb-3">Voucher Items</h4>
            <v-list class="bg-transparent">
              ...
              >
                <template #prepend>
                  <v-icon color="secondary" icon="mdi-package-variant" />
                </template>
        ...
        <v-card class="scanner-card" elevation="8">
          <div class="scanner-viewport d-flex align-center justify-center">
            <div class="text-center">
              <v-icon class="mb-2" color="grey-lighten-1" icon="mdi-qrcode-scan" size="48" />
              <h3 class="text-h6 mb-1">Scanner Hidden</h3>
              <p class="text-caption text-medium-emphasis mb-3">
                Click the eye icon to show scanner
              </p>
              <v-btn color="primary" size="small" variant="outlined" @click="toggleScanner">
                Show Scanner
              </v-btn>
            </div>
          </div>
    ...
    <!-- Stats Footer -->
    <v-row v-if="scanCount > 0" class="mt-6">
      <v-col class="mx-auto" cols="12" lg="8" md="10">
        <v-card class="text-center pa-4" variant="outlined">
          <div class="d-flex justify-space-around align-center">
            ...
            <div class="text-center">
              ...
              <div class="text-caption text-medium-emphasis">Last Scan</div>
            </div>
          </div>
        </v-card>
      </v-col>
